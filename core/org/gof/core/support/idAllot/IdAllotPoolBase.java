...
public abstract class IdAllotPoolBase {	
	private int warnNum;//剩余警戒值;		//当ID池数量小于警戒值之后 就立即申请的ID
	private int applyNum;//每次申请数量;
	private long platformDigit;//运营商标识;
	private long serverDigit;//游戏区标识;
	...
	public IdAllotPoolBase(Port port, int platformDigit, int serverDigit, int applyNum, int warnNum) {
		//记录所属Port
		this.port = port;
		this.platformDigit = platformDigit * (long) Math.pow(10, 17);
		this.serverDigit = serverDigit * (long) Math.pow(10, 13);
	...
	public long applyId() {
		...
		if(ids.isEmpty()) {
			...
			log.error("[ID分配]出现了问题：可分配ID池空了，"
					...
					applyNum, warnNum, port, new Throwable());
